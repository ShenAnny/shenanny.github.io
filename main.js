...
function genThreeYearData() //change function name for clean code
{
  ...
  {
    data_array.push([]);
    market = normal_random(5, stDevValue)
    for (fund = 0; fund < max_Fund; fund++) 
    {
      newRate = market
      if (fund == good_fund) newRate += normal_random(2, stDevValue)
      else newRate += normal_random(0, stDevValue)
...
function clearTable() 
{
  ...
  writeprofitTable()
  marketProfitTable()
  good_fund = Math.floor(max_Fund * Math.random())
  $('#goodFund').empty()
...
function method1() 
{
  ...
  for (i = 0; i < 3; i++) 
  {
    ...
    for (fund = 0; fund < max_Fund; fund++) 
    {
      ...
      if (avgArray[fund] < min && minAvgFund.indexOf(fund) == -1) 
      {
        minAvgFund[i] = fund;
        min = avgArray[fund]
      }
  ...
  if (debug) console.log(maxAvgFund, maxLast3GainFund, minAvgFund)
...
function generateData() 
{
  ...
  {
    temp = market
    if (j == good_fund) temp += normal_random(2, stDevValue)
    else temp += normal_random(0, stDevValue)
...
function marketProfitTable()
{
  ...
  //}
  //html += '</tr>'
  html += '<th>Index gain</th>'   //this will be the head of first row
...
function beatTheMarket()
{
  if (data_row == max_Year)
  {
    ...
    $('#goodFund').append("</br><b>The worthwhile fund is " + good_fund + ".</b>")
  }
}