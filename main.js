...
function gen_3year_data(){
  ...
  for (var i = 0; i < 3; i++) {
    data_array.push([]);
    temp1 = normal_random(5, Math.sqrt(200))
    for (var j = 0; j < max_row; j++) {
      temp = temp1
      if(j == good_fund){
        temp += normal_random(2, Math.sqrt(200))
      } else {
...
function writeTable() {
  ...
  if(data_row < 4) html+= '<th> New</br>Allocation(%)</th></tr>'
  else html += '<th> Current</br>Allocation(%)</th><th> Funds</br>Profits </th><th> New</br>Allocation(%) </th></tr>'
  for (var i = 0; i < max_row; i++) {
    ...
    if(data_row >= 4){
      if(user_input_array[i] === undefined) html +=''
      else html += '<td>' + user_input_array[i]+ '</td>'
      html += '<td>' + cumu_funds_array[data_row-2][i]+ '</td>'
      
    }  
      html += '<td><input style="width: 90px;"type="number" id="user_input_array[]" value=""/></td>' //this is a input box
...
function cumuFunds(){
  ...
  for(i=0;i<data_row-2;i++){
    for(j = 0; j < max_row;j++){
      cumu_funds_array.push([])
      temp = round(Number(cumu_funds_array[i][j]) * Number(100 + data_array[i+2][j]) / 100,2)
...
function method1(){
  ...
  maxIndex2++
  maxIndex3++
  if(debug) console.log(maxIndex1)
  if(debug) console.log(maxIndex2)
  if(debug) console.log(maxIndex3)
  $('#method1').append("<p>"+
  "Rank1: Fund " + maxIndex1 + "</br>" +
  "Rank2: Fund " + maxIndex2 + "</br>" +
  "Rank3: Fund " + maxIndex3 + "</br>" +
  "</p>"
  )
}